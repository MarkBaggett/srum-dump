...
def load_interfaces(reg_file):
    ...
    for eachinterface in int_keys.subkeys():
        ...
        for eachprofile in eachinterface.subkey("Profiles").subkeys():
            profileid = [x.value() for x in eachprofile.values()
                         if x.name() == "ProfileIndex"][0]
...
parser.add_argument(
    "--XLSX_OUTFILE", "-o", default="SRUM_DUMP_OUTPUT.xlsx",
...
try:
    template_wb = openpyxl.load_workbook(
        filename=options.XLSX_TEMPLATE, read_only=True)
except Exception as e:
...
for each_sheet in sheets:
    ...
    # if the table name is #XLS_CONSTANTS# then just copy the entire sheet to
    # the target workbook
    if ese_template_table == "#XLS_CONSTANTS#":
        ...
        for row_num in range(1, template_sheet.max_row + 1):
            for col_num in range(1, template_sheet.max_column + 1):
                try:
                    xls_sheet.cell(
                        row=row_num, column=col_num).value = template_sheet.cell(
                        row=row_num, column=col_num).value
                except:
                    pass
        continue
    # retrieve the names of the ESE table columns and cell styles from row 2
    # and format commands from row 3
    ese_template_fields = []
    ese_template_formats = []
    ese_template_styles = []
    # Read the first Row B & C in the template into lists so we know what data
    # we are to extract
    ...
    # Now create the worksheet in the new xls file with the same name as the
    # template
    ...
    xls_sheet = target_wb.create_sheet(title=each_sheet)
    # Now copy the header values and header formats from the template to the
    # new worksheet
    header_row = []
    ...
    # Until we get an empty row retrieve the rows from the ESE table and
    # process them
    ...
    while True:
        ...
        if ese_row is None:
            break
        # The row is retrieved now use the template to figure out which ones
        # you want and format them
        xls_row = []
        row_num += 1
        for eachcolumn, eachformat, eachstyle in zip(
                ese_template_fields, ese_template_formats, ese_template_styles):
            if eachcolumn == "#XLS_COLUMN#":
                ...
                if val == "UNABLETORETRIEVECOLUMN":
                    val = "WARNING: Invalid Column Name " + eachcolumn + " - Try one of these:" + \
                        str(ese_template_fields) + str(eachcolumn in ese_template_fields)
                    val = WriteOnlyCell(xls_sheet, value=val)
                    val.style = eachstyle
                else:
